# HarborNAS IM Gateway Agent Contract v1.1 Proposal

## Status

Recommended disposition for `HarborNAS-IM-Gateway-Agent-Contract-v1.md`:
...
- the current contract draft `HarborNAS-IM-Gateway-Agent-Contract-v1.md`
- the current HarborNAS implementation in `src/bin/assistant_task_api.rs`
- the current HarborNAS task runtime in `src/runtime/task_api.rs`
...
- direct HarborNAS-to-platform delivery is treated as transitional and should be removed after rollout
...
Current HarborNAS behavior creates or reloads task state using `task_id`. `trace_id` is useful for observability, but it is not currently the primary dedup key for task execution.
...
then HarborNAS may create a second business task run.
...
The current draft says HarborNAS owns business/task concerns, but the canonical request already includes:

- `intent.domain`
- `intent.action`

The current HarborNAS implementation dispatches directly on `domain + action`. That means v1 cannot stay ambiguous about who produces these fields.
...
- moving business intent resolution fully into HarborNAS is a v2 concern

#### Option B: Strict ownership model

- IM Gateway provides only `intent.raw_text`
- HarborNAS resolves `domain` and `action`
- HarborNAS must first implement a text-to-task entry mode before freeze
...
> In v1.1, the IM Gateway MUST populate `intent.domain` and `intent.action` for task requests because the current HarborNAS `assistant_task_api` dispatch path requires them. This does not transfer ownership of business workflow semantics to the IM Gateway; it is a transitional request-shaping requirement for compatibility with the current backend.
...
Current HarborNAS code still resolves recipients using platform-specific binding rules such as:
...
- if `route_key` is present, IM Gateway must not require HarborNAS to understand platform-native ids
...
> `destination.route_key` is the preferred outbound routing identifier in v1.1. It is an opaque IM Gateway-owned key. HarborNAS MUST treat it as write-only routing metadata and MUST NOT infer platform semantics from it.
...
- request timeout for `IM Gateway -> HarborNAS`
- request timeout for `HarborNAS -> IM Gateway`
...
- `IM Gateway -> HarborNAS`: 15s request timeout, retry only on transport failure or 5xx, never on explicit business failure
- `HarborNAS -> IM Gateway`: 10s request timeout, retry only when `retryable=true`
...
> HarborNAS MUST treat `download.url` as opaque and MUST NOT assume local filesystem access to the underlying media.
...
> HarborNAS owns business meaning. IM Gateway may adapt formatting for platform constraints, but it MUST NOT reinterpret, summarize, or alter business semantics contained in `TaskResponse` or notification payloads.
...
> `destination.route_key` is the preferred outbound routing identifier. It is opaque and IM Gateway-owned. HarborNAS MUST NOT depend on platform-specific interpretation of `route_key`.

### Attachment Access Rule

> Attachment download metadata is an opaque transport contract. HarborNAS MUST use the provided access metadata and MUST NOT assume direct filesystem access or platform-native file identifiers.

### Business Semantics Rule

> HarborNAS owns business semantics. IM Gateway may format output for a target IM platform, but MUST NOT reinterpret, summarize, or rewrite business meaning.
...
3. notification delivery using `destination.route_key` succeeds without HarborNAS providing platform-native recipient fields

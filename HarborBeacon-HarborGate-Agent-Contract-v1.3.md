# HarborBeacon IM Gateway Agent Contract v1.3
...
v1.3 freezes exactly two cross-repo interfaces:
...
- Both frozen interfaces MUST carry `X-Contract-Version: 1.3`.
...
- Async-only turn semantics are out of scope for v1.3 and belong in a later version.

### HTTP Error Envelope

For contract, auth, validation, idempotency, or infrastructure failures that are
...
```json
{
  "ok": false,
  "error": {
    "code": "AUTH_FAILED|CONTRACT_VERSION_MISMATCH|VALIDATION_ERROR|IDEMPOTENCY_CONFLICT|INFRASTRUCTURE_ERROR",
...
- 409 is reserved for idempotency or routing conflicts.
...
- `message` is required for IM Gateway callers in v1.3.
...
v1.3 adopts the transitional compatibility model:
...
- `destination.route_key` is the preferred outbound routing identifier in v1.3.
...
5. Same `task_id` replayed with conflicting task identity is rejected with `409` and `IDEMPOTENCY_CONFLICT`.
6. HarborBeacon -> IM Gateway notification send using `destination.route_key` succeeds without HarborBeacon providing platform-native recipient fields.
7. Notification retry with the same `idempotency_key` does not duplicate end-user delivery.
8. Same `idempotency_key` replayed with conflicting notification payload is rejected with `409` and `IDEMPOTENCY_CONFLICT`.
9. Expired or invalid attachment download metadata is rejected with a machine-readable error.
10. Expired or missing `route_key` returns `ROUTE_NOT_FOUND` or `ROUTE_EXPIRED`.
11. Invalid `delivery.mode` field combinations are rejected with `422` and `VALIDATION_ERROR`.
12. HarborBeacon build or contract test fails if direct IM platform credential usage remains in the notification delivery path after full cutover.
...
5. Then, add the non-frozen redacted gateway status endpoint if HarborBeacon UI needs connection state.
6. Finally, remove HarborBeacon direct IM platform delivery and direct platform credential validation so IM Gateway fully owns the IM layer.

# HarborNAS IM Gateway Agent Contract v1.3
...
6. HarborNAS -> IM Gateway notification send using `destination.route_key` succeeds without HarborNAS providing platform-native recipient fields.
...
12. HarborNAS build or contract test fails if direct IM platform credential usage remains in the notification delivery path after full cutover.
...
5. Then, add the non-frozen redacted gateway status endpoint if HarborNAS UI needs connection state.
6. Finally, remove HarborNAS direct IM platform delivery and direct platform credential validation so IM Gateway fully owns the IM layer.
